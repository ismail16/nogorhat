...
@section('title','Return Replacement')
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="table-responsive">
                            <a href="{{route('admin.return-replacement.create')}}" class="pull-right btn btn-sm btn-primary float-right ml-2"> <i class="fa fa-plus"></i> Add New</a>
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                @foreach($returns_replacements as $returns_replacement)
                                <tr>
                                    <td>{{ $loop->index+1 }}</td>
                                    <td>{{ $returns_replacement->title }}</td>
                                    
                                    <td class="text-center">
                                        <a href="{{route('admin.return-replacement.edit', $returns_replacement->id)}}"
                                        ...
                                        <form id="on_delete{{$loop->index+1}}"
                                              action="{{route('admin.return-replacement.destroy', $returns_replacement->id)}}"
                                              method="post" class="delete"
                                              data-content="{{$returns_replacement->id}}"
                                              style="display: none;">
                                            {{csrf_field()}}
                                            {{method_field('DELETE')}}
                                        </form>
